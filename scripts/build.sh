...
OPTIONS:
  ...
  -C, --codename           Codename, replaces detecting it from JOB_NAME
...
function add_md5sums_and_buildhistory_artifacts {
  ...
  for I in ${FILTERED_IMAGES}; do
    ...
    for IMG_FILE in ${ARTIFACTS}/${MACHINE}/${I}/*.vmdk* \
                    ...
                    ${ARTIFACTS}/${MACHINE}/${I}/*.wic.bmap \
                    ${ARTIFACTS}/${MACHINE}/${I}/*.wic.bz2 \
...
function move_artifacts {
  ...
  for I in ${FILTERED_IMAGES}; do
    ...
    elif ls    BUILD/deploy/images/${MACHINE}/${I}-${MACHINE}-*.wic.bz2 >/dev/null 2>/dev/null; then
      if ls    BUILD/deploy/images/${MACHINE}/${I}-${MACHINE}-*.wic.bz2 >/dev/null 2>/dev/null; then
        ln -vn BUILD/deploy/images/${MACHINE}/${I}-${MACHINE}-*.wic.bz2 ${ARTIFACTS}/${MACHINE}/${I}/
      fi
      if ls    BUILD/deploy/images/${MACHINE}/${I}-${MACHINE}-*.wic.bmap >/dev/null 2>/dev/null; then
        ln -vn BUILD/deploy/images/${MACHINE}/${I}-${MACHINE}-*.wic.bmap ${ARTIFACTS}/${MACHINE}/${I}/
      fi
      if ls    BUILD/deploy/images/${MACHINE}/${I}-${MACHINE}-*.tar.bz2 >/dev/null 2>/dev/null; then
        ln -vn BUILD/deploy/images/${MACHINE}/${I}-${MACHINE}-*.tar.bz2 ${ARTIFACTS}/${MACHINE}/${I}/
        rm -vf ${ARTIFACTS}/${MACHINE}/${I}/${I}-${MACHINE}-*.ostree.tar.bz2
      fi
...
while true ; do
  case $1 in
    ...
    -C|--codename) BUILD_CODENAME="$2" ; shift 2 ;;
